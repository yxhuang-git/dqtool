import pandas as pd
import boto3
from soda.scan import Scan
from datetime import datetime
import json
from pandas import json_normalize

# システム日時を取得する
now = datetime.now()
# S3からファイルをダウンロードする
s3_bucket = 'dqtool'
file_name = "PRODUCT_MASTER"
maker_master_file_name = "MAKER_MASTER"
s3_key = 'testdata/' + file_name + ".csv"
maker_master_s3_key = 'testdata/' + maker_master_file_name + ".csv"
s3 = boto3.client('s3')

try:
    s3.download_file(s3_bucket, s3_key, 'target.csv')
    s3.download_file(s3_bucket, maker_master_s3_key, 'reference_file.csv')
except Exception as e:
    print(f"Failed to download file: {e}")
    raise

# CSVファイルを取り込む
df_target_csv = pd.read_csv('target.csv', encoding='utf-8')
df_target_csv['開始日 > 終了日'] = df_target_csv['販売開始日'] > df_target_csv['販売終了日']
df_target_csv['定価 > 原価'] = df_target_csv['定価'] > df_target_csv['原価']
df_maker_master_csv = pd.read_csv('reference_file.csv', encoding='utf-8')
maker_id_list = df_maker_master_csv["メーカーID"].tolist()
print(df_target_csv.columns)

# Soda初期化
scan = Scan()
scan.set_scan_definition_name("dask and pandas tutorial")
scan.set_data_source_name("pandas")

# Pandas DataFrame追加
scan.add_pandas_dataframe(dataset_name="target_csv", pandas_df=df_target_csv, data_source_name="pandas")

# 検査ルール定義
row_count_checks = f"""
for each dataset target_csv:
  datasets:
    - include target_csv
  checks:
    - duplicate_count("製品 ID") = 0
    - missing_count("製品 ID") = 0
    - invalid_count("製品 ID") = 0:
        valid max length: 50
    - missing_count("メーカID") = 0
    - invalid_count("メーカID") = 0:
        valid max length: 50
        valid values: {maker_id_list}
    - missing_count("カテゴリ") = 0
    - invalid_count("カテゴリ") = 0:
        valid max length: 50
    - missing_count("サブカテゴリ") = 0
    - invalid_count("サブカテゴリ") = 0:
        valid max length: 50
    - missing_count("製品名") = 0
    - invalid_count("製品名") = 0:
        valid max length: 50
    - missing_count("原価") = 0
    - invalid_count("原価") = 0:
        valid format: integer
        valid max length: 38
    - missing_count("定価") = 0
    - invalid_count("定価") = 0:
        valid format: integer
        valid max length: 38
    - missing_count("販売開始日") = 0
    - invalid_count("販売開始日") = 0:
        valid format: date inverse
    - missing_count("販売終了日") = 0
    - invalid_count("販売終了日") = 0:
        valid format: date inverse
    - invalid_count("開始日 > 終了日") = 0:
        valid values: [false]
    - invalid_count("定価 > 原価") = 0:
        valid values: [false]
"""
scan.add_sodacl_yaml_str(row_count_checks)

# スキャンを実施して結果データをCSVに保存する
try:
    scan.execute()
    print(scan.get_logs_text())
    
    # 検査結果を取得する
    results = scan.get_scan_results()

    # json_normalizeでJSONデータへ転換
    df = json_normalize(results)

    # CSVファイルに保存して出力する
    output_file = file_name + "_CheckResult_" + now.strftime("%Y%m%d-%H%M%S") + ".csv"
    df.to_csv(output_file, index=False)

    # S3へアップロード
    s3.upload_file(output_file, s3_bucket, 'testdata/soda_check_results/' + output_file)
    print(f"Combined scan results uploaded to s3://{s3_bucket}/testdata/soda_check_results/{output_file}")

except Exception as e:
    print(f"Failed to execute scan: {e}")
    raise
