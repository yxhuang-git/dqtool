...
for each dataset target_csv:
  datasets:
    - include target_csv
  checks:
    - duplicate_count("郵便番号") = 0
    - missing_count("郵便番号") = 0
    - invalid_count("郵便番号") = 0:
        valid max length: 7
    - missing_count("都道府県名（漢字）") = 0
    - invalid_count("都道府県名（漢字）") = 0:
        valid max length: 10
    - missing_count("市区町村名（漢字）") = 0
    - invalid_count("市区町村名（漢字）") = 0:
        valid max length: 20
    - missing_count("町域名（漢字）") = 0
    - invalid_count("町域名（漢字）") = 0:
        valid max length: 50
    - missing_count("都道府県名（ローマ字）") = 0
    - invalid_count("都道府県名（ローマ字）") = 0:
        valid max length: 20
    - missing_count("市区町村名（ローマ字）") = 0
    - invalid_count("市区町村名（ローマ字）") = 0:
        valid max length: 50
    - missing_count("町域名（ローマ字）") = 0
    - invalid_count("町域名（ローマ字）") = 0:
