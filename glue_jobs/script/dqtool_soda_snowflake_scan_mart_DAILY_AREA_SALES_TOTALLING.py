...
# 検査ルール定義
row_count_checks = f"""
checks for DAILY_AREA_SALES_TOTALLING:
  - diff_cnt = 0:
      name: データレイク層にある今日発生したデータが今日の日付で取り込まれているかチェック（両方オーダー日の件数差が０と想定）
      diff_cnt query: |
        select cnt1-cnt2 from
        (select count(distinct(order_day)) as cnt1
        from DAILY_AREA_SALES_TOTALLING where order_day >= '{var_date}')a,
        (select count(distinct(order_day)) as cnt2
        from DQ_TOOL_DB.DWH_SCHEMA.TRANSACTION_DATA where order_day >= '{var_date}')b;
"""
...
try:
    ...
    # CSVファイルに保存して出力する
    output_file = "MART_SCHEMA.DAILY_AREA_SALES_TOTALLING_CheckResult_" + now.strftime("%Y%m%d-%H%M%S") + ".csv"
